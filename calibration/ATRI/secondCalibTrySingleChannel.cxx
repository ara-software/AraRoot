...
Double_t epsilon_times[2]={0}; //[capArray]
...
Int_t save_epsilon_times(char*, Int_t, Int_t);
...
Int_t calibrateDdaChan(char* baseDirName, Int_t runNum, Int_t pedNum, Double_t freq, Int_t dda, Int_t chan, bool debug=false){
  ...
  sprintf(outFileName, "%s/root/run%i/binWidthsSingleChannel.root", baseDirName, runNum);
  ...
  TH1F *lagHist[2]={0};
  for(int capArray=0;capArray<2;capArray++){
    sprintf(histName, "hist_lag_capArray%i", capArray);
    lagHist[capArray] = new TH1F(histName, histName, 200, -1.0, +1.0);
  }
  ...
  for(int entry=0;entry<numEntries;entry++){
	...
	if(retVal==0){
	  ...
	  while(TMath::Abs(deltaLag+period)<TMath::Abs(deltaLag))
	    deltaLag+=period;
	  lagTree->Fill();
	  if(TMath::Abs(noZCs[0]-noZCs[1])==0) lagHist[thisCapArray]->Fill(deltaLag);
	}
  ...
  //Calculate actual epsilon from Tree

  for(capArray=0;capArray<2;capArray++){
    char varexp[100];
    char selection[100];
    char name[100];
    sprintf(varexp, "epsilon>>tempHist");
    sprintf(selection, "thisCapArray==%i&&epsilon>-1.&&epsilon<1.", capArray);
    sprintf(name, "epsilon_hist_capArray_%i", capArray);
    epsilonTree->Draw(varexp, selection);
    histEpsilon[capArray] = (TH1D*) gPad->GetPrimitive("tempHist");
    histEpsilon[capArray]->SetName(name);
    histEpsilon[capArray]->SetTitle(name);
    histEpsilon[capArray]->Write();
    epsilon_times[capArray] = histEpsilon[capArray]->GetMean(1);
  }//capArray

  save_inter_sample_times(outFileName, dda, chan);
  save_epsilon_times(outFileName, dda, chan);
...
Int_t save_inter_sample_times(char* name, Int_t dda, Int_t channel){
  ...
  for(int capArray=0;capArray<2;capArray++) {
    ...
    for(sample=0;sample<SAMPLES_PER_BLOCK;sample++) {
      //Index values
      if(sample%2==0)
	OutFile << inter_sample_index[capArray][0][sample/2]*2 << " ";
      if(sample%2)
	OutFile << inter_sample_index[capArray][1][sample/2]*2+1 << " ";
    }
    OutFile << "\n";
    OutFile << dda << "\t" << channel << "\t" << capArray << "\t";   
    for(int sample=0;sample<SAMPLES_PER_BLOCK;sample++) {
...
Int_t save_epsilon_times(char* name, Int_t dda, Int_t channel){
  ...
  for(int capArray=0;capArray<2;capArray++){
    OutFile <<  dda << "\t"
	    << channel << "\t" 
	    << capArray << "\t";
    OutFile << epsilon_times[capArray] << "\n";
